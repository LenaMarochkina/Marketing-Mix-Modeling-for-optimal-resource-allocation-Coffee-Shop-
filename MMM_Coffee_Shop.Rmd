...
plot_sales <- function(data, y_var, title, y_label, y_breaks_by, y_limits_factor, label_prefix = "") {
  ggplot(data, aes(x = reorder(product_category, -get(y_var)), y = get(y_var), fill = product_category)) +
    ...
    scale_y_continuous(labels = comma, 
                       breaks = seq(0, max(data[[y_var]]) * y_limits_factor, by = y_breaks_by),
                       limits = c(0, max(data[[y_var]]) * y_limits_factor)) +
    labs(title = title, 
         x = "Product Category", 
         y = y_label) +
    theme_custom +
